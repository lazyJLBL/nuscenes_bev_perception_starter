...
==========================================================
使用自包含的 Simple PointPillars 检测器进行真实训练。

运行命令:
    python scripts/07_train_baseline.py --execute --epochs 1 --batch-size 2
"""
...
def main():
    args = parse_args()
    print_header("3D 检测 Baseline 训练")
...
""")
    ...
    # Merge args
    epochs = args.epochs if args.epochs is not None else det_cfg.get('epochs', 1)
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1} 完成. 耗时: {elapsed:.2f}s, Avg Loss: {metrics['loss']:.4f}")
